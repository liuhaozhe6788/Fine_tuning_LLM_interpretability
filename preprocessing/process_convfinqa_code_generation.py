"""
Script to process ConvFinQA data and generate code using OpenAI API.
...
from config.prompt_templates import four_shot_prompt_convfinqa_templates, build_finetuning_prompt_with_conversation_history
...
async def process_samples_async(
    ...
    for idx, row in df.iterrows():
        ...
        task = process_single_question_async(
            ...
            answer=answer,
            program=program,
            few_shot_examples=four_shot_prompt_convfinqa_templates,
            config=config,
            semaphore=semaphore,
    ...
    for coro in tqdm(asyncio.as_completed(tasks), total=len(tasks), desc="Processing samples asynchronously"):
        ...
        # Build fine-tuning prompt from metadata
        finetuning_prompt = build_finetuning_prompt_with_conversation_history(result.get('context', ''), result.get('question', ''), result.get('conversation_history', ''))
...
def process_convfinqa_samples(
    ...
    """
    Process ConvFinQA CSV and generate code.
    ...
    if use_async:
        ...
        for idx, row in tqdm(df.iterrows(), desc="Processing samples", total=len(df)):
            ...
            finetuning_prompt = build_finetuning_prompt_with_conversation_history(context, question, conversation_history)
            generated_code = result.get('extracted_code', '')
            actual_answer = result.get('actual_answer', '')
...
if __name__ == "__main__":
    ...
    config = CodeGenerationConfig(
        model_name="gpt-5",  # Match the model name used in generate_code.py
    ...
    process_convfinqa_samples(
        input_csv="data/clean/ConvFinQA/train.csv",
        output_csv="data/clean_with_code/ConvFinQA/convfinqa_train_generated.csv",
        config=config,
    ...
    process_convfinqa_samples(
        input_csv="data/clean/ConvFinQA/dev.csv",
        output_csv="data/clean_with_code/ConvFinQA/convfinqa_dev_generated.csv",
        config=config,
        max_concurrent=10,  # Adjust based on API rate limits (try 5-20)
        use_async=True  # Set to False for sequential processing
    )
    ...
    process_convfinqa_samples(
        input_csv="data/clean/ConvFinQA/test.csv",
        output_csv="data/clean_with_code/ConvFinQA/convfinqa_test_generated.csv",
        config=config,
        max_concurrent=10,  # Adjust based on API rate limits (try 5-20)
        use_async=True  # Set to False for sequential processing
    )
